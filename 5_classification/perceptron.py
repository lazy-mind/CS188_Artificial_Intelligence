...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        for iteration in range(iterations):
            ...
            #and print the information about our iteration
            print "Starting iteration ", iteration, "..."
            #loop over each input training data
            for i in range(len(input_train_data)):
                # the callback plots the line in the Pacman Plot
                if callback is not None: callback()
                #print "we are here ",i
                #for each training data, get a y, compared with the true, then update
                #compute the label for that training data
                result=self.classify(input_train_data[i])
                if result-(label_train_data[i]) is not 0:
                    #print "here we are"
                    self.weights[result]=self.weights[result]-input_train_data[i]
    ...
    def find_high_weight_features(self, label):
        ...
        """
        best100Features = []
        a=self.weights[label]
        #a is the list we want to sort
        
        #deal with the case if a is less than 100 element
        if len(a)<=100:
            return a
        b=sorted(a)
        #print "b:\n",b
        # now b is a sorted list
        # we just use the last 100 element
        
        end=len(b)-1
        start=len(b)-100
        while start<=end:
            best100Features.append(b[start])
            start=start+1
