...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        ...
        #print momentum
        #print batch_size
        solver=solvers.MinibatchStochasticGradientDescentSolver( learning_rate, iterations, batch_size, momentum=0, weight_decay=1e-3, shuffle=None, loss_function=None, plot=0)
        x,y=solver.solve(train_data, train_labels, val_data, val_labels, model, callback=None)
        print "done"
