...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        while a <= length-1:
            ...
            b=param_vars[a]-self.learning_rate*grad_tensors[a]
            each.append(b)
            tuple(each)
            updates.append(each)
            each=[]
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        while a <= length-1:
            each.append(param_vars[a])
            v=self.momentum*vel_vars[a]-self.learning_rate*grad_tensors[a]
            b=param_vars[a]+v
            each.append(b)
            tuple(each)
            updates.append(each)
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            #updates=session.run()
            #for a in range(len(updates)):
            updates=session.run(update_ops,feed_dict={placeholders[0]:train_data[0], placeholders[1]:train_data[1]})
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        # You may want to initialize some variables that are shared across iterations
        "*** YOUR CODE HERE ***"
        genone=MinibatchIndefinitelyGenerator(train_data,1,self.shuffle)
        gentwo=MinibatchIndefinitelyGenerator(val_data,1,self.shuffle)
        ...
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            a=genone.next()
            train_loss=session.run(loss_tensor,feed_dict={placeholders[0]:a[0], placeholders[1]:a[1]})
            updates=session.run(update_ops,feed_dict={placeholders[0]:a[0], placeholders[1]:a[1]})
            b=gentwo.next()
            val_loss=session.run(loss_tensor,feed_dict={placeholders[0]:b[0], placeholders[1]: b[1]})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        genone=MinibatchIndefinitelyGenerator(train_data,self.batch_size,self.shuffle)
        gentwo=MinibatchIndefinitelyGenerator(val_data,self.batch_size,self.shuffle)
