...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        evidenceVariablesSet=set(evidenceDict.keys())
        queryVariablesSet=set(queryVariables)
        eliminationVariable=eliminationOrder
        currentFactorsList=bayesNet.getAllCPTsWithEvidence(evidenceDict)
        
        for e in eliminationVariable:
            currentFactorsList,newFactor=joinFactorsByVariable(currentFactorsList,e)
            if len(newFactor.unconditionedVariables())!=1:
                newfactor=eliminate(newFactor,e)
                currentFactorsList.append(newfactor)
            
        fullJoint = joinFactors(currentFactorsList)
        
        tobenormalized = fullJoint
        finalCPT=normalize(tobenormalized)
        """
        print "\n\n\n","~~~~~~~~~END~~~~~~~~~","\n\n\n"
        """
        ...
        tobenormalized = incrementallyMarginalizedJoint
        finalCPT=normalize(tobenormalized)
        return finalCPT
