...
def fillObsCPT(bayesNet, gameState):
    ...
    """
        here we use the x pos to direct us of how to set the CPT
        \
        ...
        here is all about the obsVar information
        ...
        here we learn from the house case and learn how to get conditional probability
        /
    ...
    """
        if two==TOP_LEFT_VAL:
        positiontwo=topLeftPos
        if two==TOP_RIGHT_VAL:
        positiontwo=topRightPos
        if two==BOTTOM_LEFT_VAL:
        positiontwo=bottomLeftPos
        if two==BOTTOM_RIGHT_VAL:
        positiontwo=bottomRightPos
        
        if three==TOP_LEFT_VAL:
        positionthree=topLeftPos
        if three==TOP_RIGHT_VAL:
        positionthree=topRightPos
        if three==BOTTOM_LEFT_VAL:
        positionthree=bottomLeftPos
        if three==BOTTOM_RIGHT_VAL:
        positionthree=bottomRightPos
    """
    ...
    print bottomLeftPos[0]
    print bottomLeftPos[1]
    """


    bottomLeftPos, topLeftPos, bottomRightPos, topRightPos = gameState.getPossibleHouses()
    
    alist=[(TOP_LEFT_VAL,topLeftPos),(TOP_RIGHT_VAL,topRightPos),(BOTTOM_LEFT_VAL,bottomLeftPos),(BOTTOM_RIGHT_VAL,bottomRightPos)]
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for a in copy:
                ...
                #print obsVar
                #print obsPos
                dis.append((a[0],abs( a[1][0] - obsPos[0] ) + abs( a[1][1] - obsPos[1] )))
            near=['',100000]
            for a in dis:
                if(a[1]<near[1]):
                    near=a
            ...
            temp=0
            for one in OBS_VALS:
                for two in HOUSE_VALS:
                    for three in HOUSE_VALS:
                        combinations={obsVar:one,GHOST_HOUSE_VAR:two,FOOD_HOUSE_VAR:three}
                        temp=temp+1
                        #case 1
                        if two!=obsposition and three!=obsposition:
                            if one==NO_OBS_VAL:
                                obsFactor.setProbability(combinations,1)
                            if one==RED_OBS_VAL or one==BLUE_OBS_VAL:
                                obsFactor.setProbability(combinations,0)
                        #case 2
                        if two==obsposition and three!=obsposition:
                            if one==NO_OBS_VAL:
                                obsFactor.setProbability(combinations,0)
                            if one==RED_OBS_VAL:
                                obsFactor.setProbability(combinations,PROB_GHOST_RED)
                            if one==BLUE_OBS_VAL:
                                obsFactor.setProbability(combinations,1-PROB_GHOST_RED)
                        #case 3
                        if two!=obsposition and three==obsposition:
                            if one==NO_OBS_VAL:
                                obsFactor.setProbability(combinations,0)
                            if one==RED_OBS_VAL:
                                obsFactor.setProbability(combinations,PROB_FOOD_RED)
                            if one==BLUE_OBS_VAL:
                                obsFactor.setProbability(combinations,1-PROB_FOOD_RED)
                        #case 4
                        if two==obsposition and three==obsposition:
                            if one==NO_OBS_VAL:
                                obsFactor.setProbability(combinations,0)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    "*** YOUR CODE HERE ***"
    #print evidence
    newCPT=inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    pro=-100
    dic={}
    for a in newCPT.getAllPossibleAssignmentDicts():
        p=newCPT.getProbability(a)
        print p,"\n"
        if p>pro:
            pro=p
            dic=a
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        newCPT=inference.inferenceByVariableElimination(self.bayesNet,[FOOD_HOUSE_VAR,GHOST_HOUSE_VAR], evidence,eliminationOrder)
        proone=0
        protwo=0
        for a in newCPT.getAllPossibleAssignmentDicts():
            if a[FOOD_HOUSE_VAR]==HOUSE_VALS[0] and a[GHOST_HOUSE_VAR]==HOUSE_VALS[1]:
                proone=newCPT.getProbability(a)
            #food is at left
            if a[FOOD_HOUSE_VAR]==HOUSE_VALS[1] and a[GHOST_HOUSE_VAR]==HOUSE_VALS[0]:
                protwo=newCPT.getProbability(a)
            #ghost is at left
        GHOST_COLLISION_REWARD, WON_GAME_REWARD
        leftExpectedValue=proone*WON_GAME_REWARD+(protwo)*GHOST_COLLISION_REWARD
        rightExpectedValue=protwo*WON_GAME_REWARD+(proone)*GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        informationList=self.getExplorationProbsAndOutcomes(evidence)
        for a in informationList:
            x,y=self.computeEnterValues(a[1],enterEliminationOrder)
            win=max(x,y)
            expectedValue=expectedValue+a[0]*win
